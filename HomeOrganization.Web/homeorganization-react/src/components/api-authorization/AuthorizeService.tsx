...
export class AuthorizeService {
    _callbacks = [] as Callback[];
    _nextSubscriptionId = 0;
    _user: Nullable<User> = null;
    _isAuthenticated = false;
    userManager: Nullable<UserManager> = undefined;
    ...
    async signIn(state: any): Promise<AuthenticationResult> {
        ...
        return this.error("No method run")
    }
    ...
    async completeSignOut(url: string) : Promise<AuthenticationResult> {
        await this.ensureUserManagerInitialized();
        try {
            const response = await this.userManager?.signoutCallback(url);
            this.updateState(null);
            return this.success(response && response.name);
        } catch (error) {
    ...
    updateState(user: Nullable<User>) {
        this._user = user;
    ...
    error(message: string) {
        return new AuthenticationResult(AuthenticationResultStatus.Fail, message);
    }

    success(state: any) {
        return new AuthenticationResult(AuthenticationResultStatus.Success, null, state)
    }

    redirect() {
        return new AuthenticationResult(AuthenticationResultStatus.Redirect);
    }
